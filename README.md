...
Website icons load automatically, using a library of .ico files provided by DuckDuckGo. If it can’t find an icon for a website, it will load their “error” icons instead. I have been trying to write the code in a way where, if DuckDuckGo can’t provide the icon, the extension will move on to search the .ico library of another provider to load for the websites populating error icons. So far, no luck, but I will keep working on this, error icons are no fun.
