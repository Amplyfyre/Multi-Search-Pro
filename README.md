...
First, type what you want to search in the input field labeled “Type what you want to search here”.

Use the “Expand” buttons for each group category to see what websites are available, and check their individual checkboxes to include them in the overall search. A handful websites are checked by default (top-left of thumbnail), and you can check and uncheck for any batch of searches, but you can also add websites to your "Favorites ★" by clicking the star (top-right of thumbnail) to always have your favorite websites as an immediate option.

Once you have your selections set, you can click "Send" to send your search terms to the checked websites. You can also "Send ★" to send your favorited sites, or "Send All" to send every website listed in the group (careful with this one).

The “Keep popup open” option prevents the extension from closing upon sending a search. “Show unchecked” can be unchecked in order to only show the selected websites in the groups. “Throttle” helps prevent tabs from opening too fast, and “Max tabs” limits the number of websites that will open from running a search.
...
Some basic settings are available in the popup, such as hiding unnecessary groups and rearranging the order. The up and down buttons to the left of each group name will send a group one step up or down from their current location. Under the down button is an icon of an eyeball with a slash through it, that will "hide" a group and send it to the "Hidden Groups" section which can be located at the very bottom of the popup in case you want to "Unhide" them.

The rest of the settings can be found by clicking the “Settings” button at the top-right of the popup, to the right of the main search input field. You’ll be taken to a page that lists all of the websites and groups, allowing you to rewrite the labels, URLs, and add in new ones. To rearrange, drag the ☰ icon on the left of each website icon. You can also rearrange categories on the right-hand side of the page the same way. Rearrangements are implemented immediately, no need to save. Hidden groups are located at the bottom of both list of groups.

After arranging websites/groups and checking/unchecking boxes, you have the ability to save your list using the “Import/Export” feature. Click the Export button and save the compiled .json file to your computer. If you ever have to uninstall/reinstall Multi-Search Pro, click the Import button and locate the .json file to get your custom list back. This feature currently replaces all your current settings with the imported settings. I've also included the ability to export individual groups, which will import as a brand new group as opposed to wiping out your current settings.
...
• Replace only the search terms in the URL (ie. "groceries" per above example) with %s, leave everything else as is 
