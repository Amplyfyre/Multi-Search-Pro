const DEFAULTS = (function () {
  ...
  const photography = [
    { id: "p1",  name: "Everypixel",          url: "https://www.everypixel.com/search?q=%s" },
    { id: "p2",  name: "Openverse",           url: "https://openverse.org/search/image?q=%s" },
    { id: "p3",  name: "Flickr",              url: "https://www.flickr.com/search/?text=%s" },
    { id: "p4",  name: "FreeImages",          url: "https://www.freeimages.com/search/%s" },
    { id: "p5",  name: "Pexels",              url: "https://www.pexels.com/search/%s" },
    { id: "p6",  name: "Pixabay",             url: "https://pixabay.com/images/search/%s" },
    { id: "p7",  name: "Unsplash",            url: "https://unsplash.com/s/photos/%s" },

    { id: "p8",  name: "Skitterphoto",        url: "https://skitterphoto.com/photos/search/%s" },
    { id: "p9",  name: "Picjumbo",            url: "https://picjumbo.com/search/%s" },
    { id: "p10", name: "PikWizard",           url: "https://pikwizard.com/s/photo/%s" },
    { id: "p11", name: "Freepik",             url: "https://www.freepik.com/search?query=%s" },
    { id: "p12", name: "Vecteezy",            url: "https://www.vecteezy.com/search?qterm=%s&content_type=image&license-free=true" },
    { id: "p13", name: "WallHere",            url: "https://wallhere.com/en/wallpapers?q=%s" },
    { id: "p14", name: "StockFreeImages",     url: "https://www.stockfreeimages.com/p1/%s.html" },

    { id: "p15", name: "LibreShot",           url: "https://libreshot.com/?s=%s" },
    { id: "p16", name: "FocaStock",           url: "https://focastock.com/?s=%s" }
  ];
  ...
  return {
    ...
    photography: { name: "Photography (CC)", engines: photography },
...
const makeId = (prefix = "eng") => `${prefix}_${Date.now()}_${Math.floor(Math.random() * 9000) + 1000}`;
...
class ConfigManager {
  static async get() {
    try {
      ...
      for (const gid of Object.keys(config.groups)) {
        const g = config.groups[gid];
        g.assignedEngines = g.assignedEngines || (g.enabledEngines ? g.enabledEngines.slice() : []);
        g.enabledEngines = g.enabledEngines || [];
      }
      return config;
    } catch (e) {
      console.error("ConfigManager.get error", e);
      return await this.resetToDefaults();
    }
  }

  static async resetToDefaults() {
    const cfg = { engines: {}, groups: {}, customGroups: [], profiles: {}, groupOrder: [] };
    for (const [gid, group] of Object.entries(DEFAULTS)) {
      cfg.groups[gid] = { name: group.name, assignedEngines: [], enabledEngines: [] };
      for (const eng of group.engines) {
        const id = eng.id || makeId("eng");
        const icons = faviconCandidates(eng.url || "");
        cfg.engines[id] = { id, name: eng.name, url: eng.url, icon: icons[0], iconFallback: icons[1] };
        cfg.groups[gid].assignedEngines.push(id);
        if (DEFAULT_ENABLED_IDS.has(id)) cfg.groups[gid].enabledEngines.push(id);
      }
...
function domainFromUrl(u) {
  try {
    ...
    const m = u.match(/\/\/([^\/]+)/);
    return m ? m[1] : u.replace(/[^a-z0-9.-]/gi, "").slice(0, 40);
  }
}
function faviconCandidates(url) {
  const domain = encodeURIComponent(domainFromUrl(url));
  return [
...
async function createEngine(cfg, { name, url, icon }) {
  const id = makeId("eng");
  const candidates = faviconCandidates(url);
  const eng = { id, name: name || "New Engine", url, icon: icon || candidates[0], iconFallback: candidates[1] };
  cfg.engines[id] = eng;
  return eng;
}
...
async function openUrlsThrottled(urls = [], { foreground = false, throttleMs = 120, maxTabs = 500 } = {}) {
  for (let i = 0; i < urls.length; i++) {
    ...
    if (i >= maxTabs - 1) break;
    if (i < urls.length - 1 && throttleMs > 0) await new Promise((r) => setTimeout(r, throttleMs));
  }
}
  ...
  (async () => {
    try {
      if (msg.type === "GET_CONFIG") {
        sendResponse(await ConfigManager.get());
      } else if (msg.type === "SAVE_CONFIG") {
        await ConfigManager.save(msg.config);
        sendResponse({ ok: true });
      } else if (msg.type === "RESET_DEFAULTS") {
        const cfg = await ConfigManager.resetToDefaults();
        sendResponse({ ok: true, config: cfg });
      } else if (msg.type === "CREATE_ENGINE") {
        const cfg = await ConfigManager.get();
        if (!msg.url || !msg.url.includes("%s")) {
          sendResponse({ ok: false, reason: "BAD_URL" });
          return;
        }
        const eng = await createEngine(cfg, { name: msg.name, url: msg.url, icon: msg.icon });
        ...
        sendResponse({ ok: true, engine: eng });
      } else if (msg.type === "OPEN_SEARCH_TABS") {
        const cfg = await ConfigManager.get();
        const candidates = [];
        for (const gid of msg.groups || []) {
          const group = cfg.groups[gid];
          if (!group) continue;
          const visualList = group.assignedEngines || [];
          for (const eid of visualList) {
            const eng = cfg.engines[eid];
            if (!eng || !eng.url) continue;
            if (group.enabledEngines?.includes(eid)) {
              candidates.push({
                eng,
                order: eng._visualOrder ?? Infinity
              });
            }
          }
        }
        ...
        const cfg = await ConfigManager.get();
        const urls = [];
        for (const eid of msg.engines || []) {
          const eng = cfg.engines[eid];
          if (!eng || !eng.url) continue;
          urls.push(eng.url.replace("%s", encodeURIComponent(msg.query || "")));
        }
        const throttle = Number.isInteger(msg.throttleMs) ? msg.throttleMs : 0;
        await openUrlsThrottled(urls, { foreground: !!msg.foreground, throttleMs: throttle, maxTabs: Number(msg.maxTabs || 50) });
        sendResponse({ ok: true, opened: urls.length });
        ...
        try {
          ...
          const result = stored ? { ...defaults, ...stored } : defaults;
          sendResponse(result);
        } catch (e) {
      ...
      console.error("background message handler error", err, msg);
      sendResponse({ ok: false, reason: "ERROR", message: String(err) });
    }
  })();
  return true;
});
